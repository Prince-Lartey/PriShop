        ...
        <>
            <div className={`${styles.section}`}>
                <div className="hidden 800px:h-[50px] 800px:my-[20px] 800px:flex items-center justify-between">
                    <div>
                        <Link to="/">
                            <img src={PriShopLogo} alt="logo" className="w-[120px] rounded-xl"/>
                        </Link>
                    </div>
                    ...
                    <div className={`${styles.button} !rounded-[4px]`}>
                        <Link to={`${isSeller ? "/dashboard" : "/shop-create"}`}>
                            <h1 className="text-[#fff] flex items-center px-1">
            ...
            <div className={`${active === true ? "shadow-sm fixed top-0 left-0 z-10" : null} transition hidden 800px:flex items-center justify-between w-full bg-[#4d4b5a] h-[70px]`}>
                <div className={`${styles.section} relative ${styles.normalFlex} justify-between`}>
                    <div onClick={() => setDropDown(!dropDown)}>
                        <div className="relative h-[70px] w-[270px] hidden 1000px:block">
                            ...
                            <button className={`h-[100%] w-full flex justify-between items-center pl-10 bg-white font-sans text-lg font-[500] select-none`}>All Categories</button>
                            <IoIosArrowDown size={20} className="absolute right-2 top-6 cursor-pointer"onClick={() => setDropDown(!dropDown)}/>
                    ...
                    <div className="flex">
                        ...
                        <div className={`${styles.normalFlex}`}>
                            <div className="relative cursor-pointer mr-[15px]">
                                {isAuthenticated ? (
                                    <Link to="/profile">
                                        <img src={`${user.avatar.url}`} alt="" className="w-[35px] h-[35px] rounded-full border-[3px] border-[#3ad132]"/>
                                    </Link>
                                ) : (
                                    <Link to="/login">
            ...
            <div className={`${ active === true ? "shadow-sm fixed top-0 left-0 z-10" : null } w-full h-[60px] bg-[#fff] z-50 top-0 left-0 shadow-sm 800px:hidden`}>
                <div className="w-full flex items-center justify-between">
                    ...
                    <div>
                        <Link to="/">
                            <img src={PriShopLogo} alt="" className="mt-3 cursor-pointer w-[80px] rounded-xl"/>
                        </Link>
                    </div>
                    <div>
                        ...
                        <div className="fixed w-[70%] bg-[#fff] h-screen top-0 left-0 z-10 overflow-y-scroll">
                                            ...
                                            return (
                                                <Link key={index} to={`/product/${Product_name}`}>
                                                    <div className="flex items-center">
                                                        ...
                                                        <h5 className="text-[12px]">{i.name}</h5>
                                                    </div>
                                                </Link>
                                            );
                                        })}
                            ...
                            <div className="flex w-full justify-center">
                                {isAuthenticated ? (
                                    <div>
                                        <Link to="/profile">
                                            <img src={`${backend_url}${user.avatar.url}`} alt="" className="w-[60px] h-[60px] rounded-full border-[3px] border-[#0eae88]"/>
                                        </Link>
                                    </div>
                                    ) : (
                                    <>
                                        <Link to="/login" className="text-[18px] pr-[5px] text-[#000000b7] font-semibold hover:underline">
                                            Login 
                                        </Link>
                                        <Link to="/sign-up" className="text-[18px] text-[#000000b7] font-semibold hover:underline">
                                            / Sign up
                                        </Link>
                                    </>   
                                )}
